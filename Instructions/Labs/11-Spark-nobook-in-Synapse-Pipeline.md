...
8. Synapse Studio の **[開発]** ページで、 **[ノートブック]** を展開し、「**Spark Transform**」ノートブックを開きます。
9. このノートブックに含まれるコードを確認し、次の点に注意します。
...
12. **files** タブ (まだ開いているはずです) に切り替え、ルートの **files** フォルダーを表示します。 必要に応じて、 **[その他]** メニューで **[更新]** を選択して新しいフォルダーを表示します。 次に、それを開いて、Parquet ファイルがあることを確認します。
13. ルートの **files** フォルダーに戻り、ノートブックによって生成された名前が一意のフォルダーを選択し、 **[新しい SQL スクリプト]** メニューで **[上位 100 行を選択]** を選択します。
14. **[上位 100 行を選択]** ペインで、ファイルの種類を **[Parquet 形式]** に設定し、変更を適用します。
15. 新しい SQL スクリプト ペインが開くので、 **[&#9655;実行]** ボタンを使用して SQL コードを実行し、変換された販売注文データが返されることを確認します。
...
1. Synapse Studio で、ノートブックが含まれる **Spark Transform** のタブに戻り、ツール バーの右端にある **[...]** メニューで **[出力のクリア]** を選択します。
...
2. 新しいパイプラインの **[プロパティ]** ペインで、その名前を「**Pipeline1**」から「**Transform Sales Data**」に変更します。 次に、 **[プロパティ]** ペインの上にある **[プロパティ]** ボタンを使用して、それを非表示にします。
...
4. Notebook アクティビティの **[全般]** タブで、その名前を「**Run Spark Transform**」に変更します。
5. Notebook アクティビティの **[設定]** タブで、次のプロパティを設定します。
    - **Notebook**: 「**Spark Transform**」ノートブックを選択します。
    - **ベースパラメーター**: このセクションを展開し、次の設定を指定してパラメーターを定義します。
        - **名前**: folderName
        - **種類**:文字列型
...
3. パイプラインの実行が開始されたら、 **[監視]** ページで **[パイプラインの実行]** タブを表示し、「**Transform Sales Data**」パイプラインの状態を確認します。
4. 「**Transform Sales Data**」パイプラインを選択してその詳細を表示し、 **[アクティビティの実行]** ペインのパイプライン実行 ID をメモします。
