...
9. スクリプトが完了したら、出力を確認し、リソース名に対して *xxxxxxx* 形式の一意のサフィックスが生成されていることに注意してください。たとえば、作成されたリソース グループの名前は **dp203-* xxxxxxx*** です。 このサフィックスを書き留めます。後で追加のリソースを作成するときに必要になります。
...
## Azure Synapse Analytics ワークスペースについて調べる。

このスクリプトでは、Azure Synapse Analytics ワークスペースが作成されています。このワークスペースは、Azure Synapse Studio Web ベースのインターフェイスを使用して調査および管理できます。 ワークスペースには専用の SQL プールが含まれており、不要なコストが発生しないように一時停止されています。 すぐに必要になりますので、ここで再開することをお勧めします。
...
4. プールの再開には数分かかる場合があります。 **[&#8635; 最新の情報に更新]** ボタンを使用して、その状態を定期的に確認できます。 準備ができると、状態が **[オンライン]** と表示されます。 待っている間に、次の手順に進んで Lake Database を作成し、 **Manage** ページに戻り、専用 SQL プールがオンラインであることを確認します。
...
1. Azure Synapse Studio で **[データ]** ページを表示し、 **[ワークスペース]** タブで **[SQL データベース]** を展開して、ワークスペース内のデータベースを表示します。 これには、**sql*xxxxxxx*** 専用 SQL プール データベースが含まれている必要があります。
2. **Data** ペインの **+** メニューの **Lake データベース** を選択して、新しい Lake データベースをワークスペースに追加します。

    > **注**: **[OK]** ボタンをクリックする前に、読んで理解しておく必要がある **Azure Synapse データベース テンプレートの使用条件**に関するプロンプトが表示されます。

3. 新しい Lake データベースの **[プロパティ]** ペイン (右側) で、次のプロパティを設定します。
    - **名前**: lakedb
    - **入力フォルダー**: ***root/files/data** 参照する*

    >**注**: **[入力フォルダー]** を開くと、エラーが表示される可能性があります。ルート フォルダーをダブルクリックし、データに移動してから **[OK]** をクリックしてください (該当する場合)。

4. 左側の **Tables** ペインの [ **+ テーブル** ] メニューで、[データ レイクから ***] を選択します***。 次のプロパティを持つ新しいテーブルを追加します。
    - ***外部テーブル名***: 製品
    - ***[リンク サービス]*** : synapse*xxxxxxx*-WorkspaceDefautStorage(datalake*xxxxxxx*) を選択します
    - ***入力ファイルまたはフォルダー***: files/data/products.csv

5. **[続行]** をクリックし、**[新しい外部テーブル]** ペインで、[最初の行] を選択して***列名を推論し***、**[作成]** をクリックします。

6. レイク データベース ウィンドウの上部にある **[発行]** を選択して変更を保存します
7. **[データ]** ペインで、**Lake データベース** セクションを展開して、**lakedb** を展開し、次に **製品** テーブルの **...** メニューで、**[SQL スクリプトの作成** > ***Top 100 行***を選択します。
8. **Connect to** が **Built-in** として一覧表示されていることを確認し、**ユーザー データベース**一覧を最新の情報に更新して、**lakedb** を選択します。

![レイク データベースの最初の外部クエリ](./images/lakedb-first-external-query.png)

9. **[実行]** ボタンを使用してクエリを実行し、**製品** テーブル内のデータを表示します。

## Microsoft Purview サービス アカウントを追加して構成する
...
### Microsoft Purview アカウントをプロビジョニングする

> **注**: Purview リソースは、1 つの Azure テナント内で制限されます。 使用しているテナントで Azure Purview インスタンスのクォータが既に使用されている場合、新しいテナントを作成することはできません。 可能であれば、この演習の残りの部分で既存の Microsoft Purview リソースを使用できます。

1. Azure portal を開いている [ブラウザー] タブに戻り、**dp203-*xxxxxxx*** リソース グループを表示します。
2. [**+ 作成**] ボタンを使用して、次の設定で新しい **Microsoft Purview** リソースをリソース グループに追加します。
    - **サブスクリプション**: ''ご利用のサブスクリプションを選択します''**
    - **リソース グループ**: dp203-*xxxxxxx*
    - **Microsoft Purview アカウント名**: purview*xxxxxxx**(ここで、 *xxxxxxx* は一意のサフィックスです)*
    - **[場所]**: *使用できるリージョンを選択する*

    > **注**: Purview で検証に合格できるようにするには、いくつかのリージョンを試す必要がある場合があります。

3. リソースが作成されるまで待ってから、**dp203-*xxxxxxx*** リソース グループに戻り、それが一覧表示されていることを確認します (ページを更新する必要がある場合があります)。
...
1. **dp203-*xxxxxxx*** リソース グループで、作成したリソースを確認します。 これには以下が含まれます。
...
1. Azure Synapse Studio ブラウザー タブに戻ります。 次に、**[データ]** ページを表示して、ワークスペース内のデータベースを表示します。 これらに、以下が含まれている必要があります。
    - **lakedb** という名前の レイク データベース。
    - **sql*xxxxxxx*** という名前の専用 SQL プール データベース。

2. **lakedb** データベースを選択し、その **[...]** メニューで **[新しい SQL スクリプト]**  >  **[空のスクリプト]** を選択して、新しい **[SQL スクリプト 1]** ペインを開きます。 ツール バーの右端にある **[プロパティ]** ボタン ( **&#128463;<sub>*</sub>** のような外観) を使用して **[プロパティ]** ペインを非表示にし、スクリプト ペインをより簡単に表示できます。
...
3. **[&#9655; 実行]** ボタンを使用してスクリプトを実行すると、Microsoft Purview によって使用されるマネージド ID の **lakedb** ユーザーにログインとユーザーが作成され、そのユーザーが **lakedb** データベースの **db_datareader** ロールに追加されます。
4. **sql*xxxxxxx*** 専用 SQL プール データベースに新しい空のスクリプトを作成し、それを使用して次の SQL コードを実行する (***purviewxxxxxxx*** をご自分の Microsoft Purview アカウントのマネージド ID 名に置き換えます) と、Microsoft Purview によって使用されるマネージド ID の専用 SQL プールにユーザーが作成され、**sql*xxxxxxx*** データベースの **db_datareader** ロールに追加されます。
...
## Microsoft Purview を使用してデータ リソースをスキャンする
...
Microsoft Purview を使用すると、Azure Synapse ワークスペース内のデータ ソースを含め、データ エステート全体でデータ資産をカタログ化できます。 デプロイしたばかりのワークスペースには、データ レイク (Azure Data Lake Storage Gen2 アカウント内)、lake データベース、専用 SQL プール内のデータ ウェアハウスが含まれます。
