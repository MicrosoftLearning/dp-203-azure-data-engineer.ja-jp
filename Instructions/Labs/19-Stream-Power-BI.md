...
3. わかりやすい名前 (**mslearn-streaming** など) を指定し、*Pro* ライセンス モードを選んで、新しいワークスペースを作成します。
...
1. Azure portal が表示されているブラウザー タブに戻り、スクリプトが完了したら、**dp203-*xxxxxxx*** リソース グループがプロビジョニングされたリージョンを記録しておきます。
...
1. **stream-orders** の概要ページで、 **[入力]** ページを選び、 **[入力の追加]** メニューを使って、次のプロパティを含む**イベント ハブ**入力を追加します。
    - **入力エイリアス**: `orders`
...
1. **stream-orders** Stream Analytics ジョブの **[出力]** ページを表示します。 次に、 **[出力の追加]** メニューを使って、次のプロパティを含む **Power BI** 出力を追加します。
...
1. PowerBI のブラウザー タブに戻ります。

2. ワークスペースの **[+ 新規]** ドロップダウン メニューで、 **[ダッシュボード]** を選択し、**注文の追跡**という名前の新しいダッシュボードを作成します。

3. **注文の追跡**ダッシュボードで **&#9999;&#65039; [編集]** メニューを選んでから、 **[+ タイルの追加]** を選びます。 次に、 **[タイルの追加]** ペインで **[カスタム ストリーミング データ]** を選んで、 **[次へ]** を選びます。

4. **[カスタム ストリーミング データ タイルの追加]** ペインの **[データセット]** で、**realtime-data** データセットを選び、 **[次へ]** を選びます。

5. 既定の視覚化の種類を **[折れ線グラフ]** に変更します。 次に、以下のプロパティを設定して、 **[次へ]** を選びます。
    - **軸**: EndTime
    - **値**: Orders
    - **表示する時間枠**: 1 分

6. **[タイルの詳細]** ペインで、 **[タイトル]** を「**リアルタイム注文数**」に設定して、 **[適用]** を選びます。

7. Azure portal を含むブラウザー タブに戻り、必要に応じてクラウド シェル ペインを再度開きます。 次に、次のコマンドを再実行して、別の 100 件の注文を送信します。

    ```
    node ~/dp-203/Allfiles/labs/19/orderclient
    ```

8. 注文送信スクリプトの実行中に、**注文の追跡** Power BI ダッシュボードを含むブラウザー タブに戻り、Stream Analytics ジョブ (まだ実行中) による処理時に新しい注文データが反映されるように視覚化が更新されることを確認します。
